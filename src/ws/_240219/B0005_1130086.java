...
public class B0005_1130086 {

	// 닉네임을 사용자에 맞게 변경해 주세요.
	static final String NICKNAME = "B0005_1130086";
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (socket != null) {
				...
				float targetBall_x = balls[1][1], targetBall_y = balls[1][1];
				...
				float ballAngle = 0, holeAngle = 0;
				if(N >= 6) {
					for (int i = 0; i < HOLES.length; i++) {
						// 흰공과 목적구 사이의 각도
						double temp = height > 0? Math.atan(width / height) : 0;
						ballAngle = (float) ((180.0 / Math.PI) * temp);
						
						float tempX = Math.abs(whiteBall_x - HOLES[i][0]);
						float tempY = Math.abs(whiteBall_y - HOLES[i][1]);
						
						// 흰공과 구멍 사이의 각도
						temp = tempX > 0? Math.atan(tempX / tempY) : 0;
						holeAngle = (float) ((180.0 / Math.PI) * temp);
						
						gap = Float.min(gap, Math.abs(holeAngle - ballAngle));
					}
					if(gap > 2) {
						targetBall_y -= 3;
						targetBall_x -= 3;
...
//				
//				  if (Math.abs(angle - ballAngle) < 30) {
//		                angle += (angle - ballAngle) / distance * 10;
//		            }
