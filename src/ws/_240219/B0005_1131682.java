...
public class B0005_1131682 {

	/*
	 * 
	 * 대전 5반 김정현 전략 : 1, 2, 3, 4사분면 별로 타겟공을 일직선으로 보낼 수 있도록 하였습니다.
	 * 
	 */

	// 닉네임을 사용자에 맞게 변경해 주세요.
	static final String NICKNAME = "B0005_1131682";
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (socket != null) {
				...
				System.out.println("target ball : " + targetBall_x + " " + targetBall_y);
				for (int i = 1; i < NUMBER_OF_BALLS; i++) {
					if (balls[i][0] > 0 && balls[i][0] < 255 && balls[i][1] > 0 && balls[i][1] < 125) {
				...
				/*
				 * int a = math.abs(end[0] - start[0]); // x 좌표의 차이 int b = math.abs(end[1] -
				 * start[1]); // y 좌표의 차이
				 * 
				 * double c = math.sqrt(math.pow(a,2) + math. pow(b,2)); double radian =
				 * math.atan(b / a); math.todegrees(radian); a^2 = b^2 + c^2 - 2abcosC
				 * 
				 */
				...
				if (whiteBall_x < targetBall_x && whiteBall_y > targetBall_y) {
					radian = Math.atan(width / height);
					angle = (float) (180.0 - ((180.0 / Math.PI) * radian));
				}
